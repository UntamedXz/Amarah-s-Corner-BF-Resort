...
foreach ($getSubcategory as $subcategoryRow) {
                ?>
    <option value="<?php echo $subcategoryRow['subcategory_id']; ?>"><?php echo $subcategoryRow['subcategory_title']; ?>
    </option>
    <?php
}
            ?>
...
if (isset($_POST['insert_simple'])) {
    ...
    if ($product_id_saved == null) {
        ...
        $get_product_type_db = mysqli_query($conn, "SELECT * FROM product WHERE product_id = $product_id_saved");

        $get_product_type = mysqli_fetch_array($get_product_type_db);

        $product_type_db = $get_product_type['product_type'];

        if ($product_type != $product_type_db) {
            $delete_attribute_db = mysqli_query($conn, "DELETE FROM product_attribute WHERE product_id = $product_id_saved");
...
if (isset($_POST['update_simple'])) {
    ...
    if ($product_type != $product_type_db) {
        ...
        if ($_FILES['product_image']['error'] === 4) {
            ...
            if ($update_product) {
                $product_id = mysqli_insert_id($conn);
                echo 'update-product?id=' . $product_id_saved;
            }
        } else {
            $img_ext = explode('.', $product_image);
            $img_ext = strtolower(end($img_ext));
...
if (isset($_POST['update_variable'])) {
    ...
    $product_desc = $_POST['product_desc'] ?? null;
    $product_slug = $_POST['product_slug'] ?? null;
    $product_status = $_POST['stock_status'] ?? null;
    $attribute_name = $_POST['attribute_name'] ?? null;
    $product_type = $_POST['product_type'];
    $variation_id = $_POST['variation_id'] ?? null;
    $variation_name = $_POST['variation_name'] ?? null;
